...
-- Run ONLY when AUTH_REQUIRED = "true".
...
alter table public.lockin_inbox enable row level security;
...
drop policy if exists "own_inbox" on public.lockin_inbox;
create policy "own_inbox" on public.lockin_inbox
for all to authenticated using (auth.uid()::text = user_id) with check (auth.uid()::text = user_id);

drop policy if exists "own_reviews" on public.weekly_reviews;
create policy "own_reviews" on public.weekly_reviews
for all to authenticated using (auth.uid()::text = user_id) with check (auth.uid()::text = user_id);
